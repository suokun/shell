...
echo "max 99 latency"
grep "sockperf: ---> percentile 99.000" ./$var | awk ' { print ( $(NF) ) }'  | awk 'BEGIN {max = 0} {if ($1+0 > max+0) max=$1} END {print "Max=", max}'

echo "max 99.9 latency"
grep "sockperf: ---> percentile 99.900" ./$var | awk ' { print ( $(NF) ) }'  | awk 'BEGIN {max = 0} {if ($1+0 > max+0) max=$1} END {print "Max=", max}'

echo "max std-dev"
grep "std-dev" ./$var | awk ' { print ( $(NF) ) }' > t0
sed -n 's/^(std-dev=//p' t0 > t1
sed -n 's/)//p' t1 | awk 'BEGIN {max = 0} {if ($1+0 > max+0) max=$1} END {print "Max=", max}'
